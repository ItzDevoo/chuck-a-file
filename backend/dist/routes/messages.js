...
router.post('/send', authenticateToken, async (req, res) => {
    ...
    `, [req.userId, recipientId, recipientId, req.userId]);
        ...
        // Insert message
        const result = await database_1.db.run('INSERT INTO messages (sender_id, recipient_id, message_text, message_type) VALUES (?, ?, ?, ?)', [req.userId, recipientId, message, messageType]);
        const newMessage = await database_1.db.get('SELECT * FROM messages WHERE id = ?', [Number(result.lastInsertRowid)]);
        res.status(201).json({
            success: true,
...
router.get('/conversation/:friendId', authenticateToken, async (req, res) => {
    ...
    `, [req.userId, friendId, friendId, req.userId]);
        ...
        // Get messages
        const messages = await database_1.db.all(`
...
router.get('/conversations', authenticateToken, async (req, res) => {
    try {
        const conversations = await database_1.db.all(`
      SELECT 
        u.id as friend_id,
